...
pagetitle: "Intro DS - 99-formato-test"
subtitle: "99-formato-test"
...
source("R/00-xaringan-knitr-setup.R")
```

## Metadata

```{r}
dplyr::glimpse(rmarkdown::metadata)
```
...
# ggplot(gapminder |> dplyr::filter(year >= 1992)) +
ggplot(gapminder |> dplyr::filter(year  == max(year))) +  
  ...
  labs(
    title = "Un Título",
    subtitle = "Texto un poco más largo",
    x = "Variable eje x",
    y = "Valor variable horizontal",
    caption = "Esto será el origen de datos o algo similar?"
    )
```
...
```{r eval=requireNamespace("showtext", quietly=TRUE) && requireNamespace("ggplot2", quietly=TRUE)}
